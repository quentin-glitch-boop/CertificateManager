...
class TestDocumentCreation:
    ...
    def test_add_document_missing_title(self, logged_in_client):
        ...
        assert b'titre est obligatoire' in response.data or b'Le titre est obligatoire' in response.data or b'titre' in response.data.lower()
    ...
    def test_add_document_missing_pdf(self, logged_in_client):
        ...
        assert b'PDF est obligatoire' in response.data or b'Le fichier PDF est obligatoire' in response.data or b'fichier' in response.data.lower()
...
class TestUserManagement:
    ...
    def test_admin_users_page_requires_admin(self, logged_in_client):
        ...
        assert b'Acces refuse' in response.data or response.status_code == 403 or response.request.path == '/'
    ...
    def test_add_user_as_admin(self, admin_client):
        ...
        assert response.status_code == 200
        assert b'Utilisateur ajoute' in response.data or b'ajoute' in response.data or b'success' in response.data.lower()
    ...
    def test_add_user_short_password(self, admin_client):
        ...
        assert b'6 caracteres' in response.data.lower() or b'caracteres' in response.data.lower() or b'mot de passe' in response.data.lower()
